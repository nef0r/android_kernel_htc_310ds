...
/* [CCI] Touch - Start */
struct tp_fw_info {
	u8 shipping_fw_version;         /* shipping firmware version */
	u8 tp_id;                       /* TP ID (Module ID) */
	unsigned char module_name[30];  /* TP module name */
};

int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth);

static unsigned char CTPM_FW_0[] = {
	//#include "ft_app.i"
//	#include "FT6x06_4Column_V04_20131122_app.i"
//	#include "FT6x06_4Column_V20_HSD_20131204_app.i"
//	#include "FT6x06_4Column_V30_0x24_20131213_app.i"
//	#include "FT6x06_4Column_V30_0x27_20131217_app.i"
//	#include "FT6x06_4Column_V30_0x29_20131226_app.i"
//	#include "FT6x06_4Column_V30_0x30_20140108_app.i"
//	#include "FT6x06_4Column_V30_0x33_20140115_app.i"
//	#include "FT6x06_4Column_V30_0x34_20140116_app.i"
	#include "FT6x06_4Column_V30_0x36_20140122_app.i"
};

static unsigned char CTPM_FW_1[] = {
	#include "FT6x06_4Column_AVC_0xD0_htc_20140207_app.i"
};

#define TP_SOURCE_NUM 2

static struct tp_fw_info tp_modules[] = {
        {
                .shipping_fw_version = 0x36,
                .tp_id = 0x5A,
                .module_name = "Truly",
        },   
        {
                .shipping_fw_version = 0xD0,
                .tp_id = 0x8E,
                .module_name = "AVC",
        },   
};

static int tp_module_id = 0;
/* [CCI] Touch - End */
...
#define FT5x0x_REG_PANEL_ID	0xA8
		    ...
		    int writelen, char *readbuf, int readlen)
{
	...
	if (writelen > 0) {
		...
		if (ret < 0)
			dev_err(&client->dev, "[FOCAL] %s: i2c read error.\n",
				__func__);
	} else {
		struct i2c_msg msgs[] = {
			{
...
int ft5x0x_dma_i2c_Write(struct i2c_client *client, u32 writebuf, int writelen)
{
	...
	if (ret < 0)
		dev_err(&client->dev, "[FOCAL] %s i2c write error.\n", __func__);
...
{
	u8 *pbt_buf = (tp_module_id) ? CTPM_FW_1 : CTPM_FW_0;
	int i_ret;
	int fw_len = (tp_module_id) ? sizeof(CTPM_FW_1) : sizeof(CTPM_FW_0);

	/*judge the fw that will be upgraded
	* if illegal, then stop upgrade and return.
	*/
	if (fw_len < 8 || fw_len > 32 * 1024) {
		dev_err(&client->dev, "[FOCAL] %s:FW length error, fw_len = %d\n", __func__, fw_len);
		return -EIO;
	}
	...
	if ((pbt_buf[fw_len - 8] ^ pbt_buf[fw_len - 6]) == 0xFF
		...
		if (i_ret != 0)
			dev_err(&client->dev, "[FOCAL] %s:upgrade failed. err.\n",
					__func__);
#ifdef AUTO_CLB
		else
		...
		dev_err(&client->dev, "[FOCAL] %s:FW format error\n", __func__);
		return -EBADFD;
	}
...
{
	u16 ui_sz;
	ui_sz = (tp_module_id) ? sizeof(CTPM_FW_1) : sizeof(CTPM_FW_0);
	u8 *pbt_buf = (tp_module_id) ? CTPM_FW_1 : CTPM_FW_0;
...
/* [OPT] Read panel id from .i file, 20131204, Add Start */
u8 fts_ctpm_get_i_file_panel_id(void)
{
	u16 ui_sz;
	ui_sz = (tp_module_id) ? sizeof(CTPM_FW_1) : sizeof(CTPM_FW_0);
	u8 *pbt_buf = (tp_module_id) ? CTPM_FW_1 : CTPM_FW_0;
	...
	if(ui_sz > 2)
		return pbt_buf[ui_sz - 1];

	return 0x00;
}
/* [OPT] Read panel id from .i file, 20131204, Add End */
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	*/
	...
	if (reg_val[0] == 0x79 && reg_val[1] == 0x6)
		dev_dbg(&client->dev, "[FOCAL][FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
			 reg_val[0], reg_val[1]);
	else
		return -EIO;
	...
	dev_dbg(&client->dev, "[FOCAL] bootloader version = 0x%x\n", reg_val[0]);
	...
	dev_dbg(&client->dev, "[FOCAL][FTS] old setting: uc_i2c_addr = 0x%x,\
...
/* [CCI] Touch - Start */
int fts_read_tp_id_and_fw_ver(struct i2c_client *client, u8 *fw_ver,
		u8 *tp_id, u8 *shipping_fw_ver)
{
	int ret = 0;
	int i;
	int compatibled = 0;

	ret = ft5x0x_read_reg(client, FT5x0x_REG_FW_VER, fw_ver);
	if (ret < 0) {
		printk("[FOCAL][FTS] %s: read fw version failed\n", __func__);
		return ret;
	}

	ret = ft5x0x_read_reg(client, FT5x0x_REG_PANEL_ID, tp_id);
	if (ret < 0) {
		printk("[FOCAL][FTS] %s: read panel id failed\n", __func__);
		return ret;
	}

	printk("[FOCAL][FTS] %s: fw_ver = 0x%02x, tp_id = 0x%02x\n",
		__func__, *fw_ver, *tp_id);

	/* use TP ID to find the compatible firmware data */

	for (i = 0; i < TP_SOURCE_NUM; i++) {
		if (tp_modules[i].tp_id == *tp_id) {
			tp_module_id = i;
			*shipping_fw_ver = tp_modules[i].shipping_fw_version;
			compatibled = 1;
		}
	}

	if (compatibled == 0) {
		printk("[FOCAL][FTS] %s: cannot find the compatible firmware, "
			"use main source fw as default\n", __func__);
		/* use main source firmware */
		tp_module_id = 0;
		*shipping_fw_ver = tp_modules[0].shipping_fw_version;
	}

	printk("[FOCAL][FTS] %s: [TP Module info] "
		"Module Name (%s), Shipping FW Version (0x%02x), "
		"TP ID (0x%02x)\n",
		__func__, tp_modules[tp_module_id].module_name,
		tp_modules[tp_module_id].shipping_fw_version,
		tp_modules[tp_module_id].tp_id);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_tp_id_and_fw_ver(client, &uc_tp_fm_ver, &uc_tp_tp_id, &shipping_fw_ver);

	uc_host_fm_ver = fts_ctpm_get_i_file_ver();

	printk("[FOCAL][FTS] tp_module_id = %d, uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
		tp_module_id, uc_tp_fm_ver, uc_host_fm_ver);
...
#if 0
	if(uc_tp_fm_ver != CCI_CURRENT_FW)
	{
#endif
		msleep(100);
		dev_dbg(&client->dev, "[FOCAL][FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
				uc_tp_fm_ver, uc_host_fm_ver);
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
		if (i_ret == 0)	{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			dev_dbg(&client->dev, "[FOCAL][FTS] upgrade to new version 0x%x\n",
					uc_host_fm_ver);
		} else {
			pr_err("[FOCAL][FTS] upgrade failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}
			  ...
			  u32 dw_lenth)
{
	...
	if(!gpDMABuf_va){
        DBG("[FOCAL][Error] Allocate DMA I2C Buffer failed!\n");
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		/*********Step 1:Reset  CTPM *****/
		/*write 0xaa to register 0xfc */
		printk("[FOCAL][FT5606] [Step 1:Reset  CTPM]\n");
		if (DEVICE_IC_TYPE == IC_FT6208 || DEVICE_IC_TYPE == IC_FT6306)
		...
		delay_qt_ms(30);
		/*********Step 2:Enter upgrade mode *****/
		printk("[FOCAL][FT5606] [Step 2:Enter upgrade mode]\n");
		...
		/*********Step 3:check READ-ID***********************/
		printk("[FOCAL][FT5606] [Step 3:check READ-ID]\n");
		auc_i2c_write_buf[0] = 0x90;
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] =
			0x00;
		ft5x0x_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);
		...
		if (reg_val[0] == upgradeinfo.upgrade_id_1
			...
			//dev_dbg(&client->dev, "[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				//reg_val[0], reg_val[1]);
			DBG("[FOCAL][FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
			dev_err(&client->dev, "[FOCAL][FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
		}
	}
	...
	printk("[FOCAL][FT5606] [Step 4:erase app and panel paramenter area]\n");
	DBG("[FOCAL] Step 4:erase app and panel paramenter area\n");
	...
	printk("[FOCAL][FT5606] [Step 5:write firmware(FW) to ctpm flash]\n");
	bt_ecc = 0;
	DBG("[FOCAL] Step 5:write firmware(FW) to ctpm flash\n");
	...
	printk("[FOCAL][FT5606] [Step 6: read out checksum]\n");
	DBG("[FOCAL] Step 6: read out checksum\n");
	auc_i2c_write_buf[0] = 0xcc;
	ft5x0x_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	if (reg_val[0] != bt_ecc) {
		dev_err(&client->dev, "[FOCAL][FTS]--ecc error! FW=%02x bt_ecc=%02x\n",
					reg_val[0],
					bt_ecc);
		if(gpDMABuf_va){
	...
	printk("[FOCAL][FT5606] [Step 7: reset the new FW]\n");
	DBG("[FOCAL] Step 7: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	ft5x0x_i2c_Write(client, auc_i2c_write_buf, 1);
			  ...
			  u32 dw_lenth)
{
	...
	/*********Step 1:Reset  CTPM *****/
	/*write 0xaa to register 0xfc */
	if (IC_FT6208 == DEVICE_IC_TYPE || IC_FT6306 == DEVICE_IC_TYPE)
		ft5x0x_write_reg(client, 0xbc, FT_UPGRADE_AA);
	else
		ft5x0x_write_reg(client, 0xfc, FT_UPGRADE_AA);
				       ...
				       char *firmware_name)
{
	...
	if (fwsize <= 0) {
		dev_err(&client->dev, "[FOCAL] %s ERROR:Get firmware size failed\n",
					__func__);
		return -EIO;
	}
	...
	if (ft5x0x_ReadFirmware(firmware_name, pbt_buf)) {
		dev_err(&client->dev, "[FOCAL] %s() - ERROR: request_firmware failed\n",
					__func__);
		kfree(pbt_buf);
		return -EIO;
	}
	...
	if ((pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]) == 0xFF
		...
		if (i_ret != 0)
			dev_dbg(&client->dev, "[FOCAL] %s() - ERROR:[FTS] upgrade failed..\n",
						__func__);
		else {
#ifdef AUTO_CLB
			fts_ctpm_auto_clb(client);	/*start auto CLB*/
		...
		dev_dbg(&client->dev, "[FOCAL] %s:FW format error\n", __func__);
		kfree(pbt_buf);
		return -EIO;
	}
					...
					const char *buf, size_t count)
{
	...
	if (0 != retval) {
		dev_err(&client->dev, "[FOCAL] %s() - ERROR: Could not convert the "\
	...
	if (2 == num_read_chars) {
		...
		if (ft5x0x_read_reg(client, regaddr, &regvalue) < 0)
			dev_err(&client->dev, "[FOCAL] Could not read the register(0x%02x)\n",
						regaddr);
		else
			pr_info("[FOCAL] the register(0x%02x) is 0x%02x\n",
					regaddr, regvalue);
	} else {
		regaddr = wmreg >> 8;
		regvalue = wmreg;
		if (ft5x0x_write_reg(client, regaddr, regvalue) < 0)
			dev_err(&client->dev, "[FOCAL] Could not write the register(0x%02x)\n",
							regaddr);
		else
			dev_err(&client->dev, "[FOCAL] Write 0x%02x into register(0x%02x) successful\n",
							regvalue, regaddr);
	}
					...
					const char *buf, size_t count)
{
	...
	if (i_ret == 0) {
		msleep(300);
		uc_host_fm_ver = fts_ctpm_get_i_file_ver();
		pr_info("[FOCAL] %s [FTS] upgrade to new version 0x%x\n", __func__,
					 uc_host_fm_ver);
	} else
		dev_err(&client->dev, "[FOCAL] %s ERROR:[FTS] upgrade failed.\n",
					__func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (fts_read_tp_id_and_fw_ver(client, &fwver, &tp_id, &shipping_fw_ver) < 0) {
		num_read_chars = snprintf(buf, PAGE_SIZE,
				"get tp fw version fail!\n");
	} else {
		printk("[FOCAL] %s: fw version = 0x%02x, expected version = 0x%02x\n",
			__func__, fwver, shipping_fw_ver);
		num_read_chars = snprintf(buf, PAGE_SIZE, "%d\n", (fwver == shipping_fw_ver) ? 1 : 0);
	}
...
/* [LX83] Implement touch auto firmware upgrade, 20131125, Add Start */
static ssize_t ft5x0x_fw_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	u8 uc_host_fm_ver = FT5x0x_REG_FW_VER;
	u8 uc_i_file_fm_ver = 0x00;
	u8 uc_host_fm_panel_id = 0x00;
	ssize_t num_read_chars = 0;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);

	u8 shipping_fw_ver;

	fts_read_tp_id_and_fw_ver(client, &uc_host_fm_ver, &uc_host_fm_panel_id, &shipping_fw_ver);

//	ft5x0x_read_reg(client, FT5x0x_REG_FW_VER, &uc_host_fm_ver);
//	ft5x0x_read_reg(client, FT5x0x_REG_PANEL_ID, &uc_host_fm_panel_id);
	uc_i_file_fm_ver = fts_ctpm_get_i_file_ver();

	printk("[TP] uc_i_file_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", uc_i_file_fm_ver, uc_host_fm_ver);

//	if(uc_host_fm_panel_id == MAIN_PANEL_ID)
//	{
		if (uc_host_fm_ver == FT5x0x_REG_FW_VER || uc_host_fm_ver < uc_i_file_fm_ver )
//		if(uc_host_fm_ver != CCI_CURRENT_FW)
			num_read_chars += sprintf(&(buf[num_read_chars]), "1\n");
		else
			num_read_chars += sprintf(&(buf[num_read_chars]), "0\n");
//	}

	return num_read_chars;
}
static DEVICE_ATTR(check_upgrade_result, 0644, ft5x0x_fw_check_show, NULL);

static ssize_t ft5x0x_auto_upgrade_show(struct device *dev, struct device_attribute *attr, char *buf, size_t size)
{
        struct i2c_client *client = container_of(dev, struct i2c_client, dev);
        int ret = 0;
        ssize_t num_read_chars = 0;

        ret = fts_ctpm_auto_upgrade(client);
        if(ret<0)
                num_read_chars += sprintf(&(buf[num_read_chars]), "0\n");
        else
                num_read_chars += sprintf(&(buf[num_read_chars]), "1\n");
...
/* [LX83] Implement touch auto firmware upgrade, 20131125, Add End */

/* [LX83] Implement reading panel id, 20131204, Add Start */
static ssize_t ft5x0x_panel_id_show(struct device *dev, struct device_attribute *attr, char *buf, size_t size)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	ssize_t num_read_chars = 0;

	u8 uc_host_fm_panel_id = 0x00;
	u8 uc_i_file_fm_panel_id = 0x00;

	ft5x0x_read_reg(client, FT5x0x_REG_PANEL_ID, &uc_host_fm_panel_id);
	uc_i_file_fm_panel_id = fts_ctpm_get_i_file_panel_id();

	return num_read_chars += sprintf(&(buf[num_read_chars]),
			"host panel id=0x%x, i file panel id=0x%x, tp_module_id=%d\n", uc_host_fm_panel_id, uc_i_file_fm_panel_id, tp_module_id);
}
static DEVICE_ATTR(panel_id, 0644, ft5x0x_panel_id_show, NULL);
/* [LX83] Implement reading panel id, 20131204, Add End */
...
int ft5x0x_create_sysfs(struct i2c_client *client)
{
	...
	if (0 != err) {
		...
		pr_info("[FOCAL] ft5x0x:%s() - sysfs_create_group() succeeded.\n",
				__func__);
	}
	return err;
}
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	case PROC_UPGRADE:
		{
			...
			enable_irq(client->irq);
			if (ret < 0) {
				dev_err(&client->dev, "[FOCAL] %s:upgrade failed.\n", __func__);
				return ret;
			}
		}
		break;
	case PROC_READ_REGISTER:
		writelen = 1;
		DBG("[FOCAL] %s:register addr=0x%02x\n", __func__, writebuf[1]);
