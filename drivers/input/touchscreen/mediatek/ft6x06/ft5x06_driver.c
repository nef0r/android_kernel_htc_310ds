...
#define APS_ERR(fmt,arg...)           printk("[FOCAL] <<proximity>> "fmt"\n",##arg)
#define TPD_PROXIMITY_DEBUG(fmt,arg...)           printk("[FOCAL] <<proximity>> "fmt"\n",##arg)
#define TPD_PROXIMITY_DMESG(fmt,arg...)           printk("[FOCAL] <<proximity>> "fmt"\n",##arg)

#define TPD_PROXIMITY_REG	0x01
static u8 tpd_proximity_flag = 0;
static u8 tpd_proximity_flag_one = 0; //add for tpd_proximity by wangdongfang
static u8 tpd_proximity_detect = 1; //0-->close ; 1--> far away

#define TPD_POWER_MODE_REG      0xB0 //HUANG
#endif

#define FTS_APK_DEBUG
#define SYSFS_DEBUG
/* [OPT][Touch] Add for touch debug, 20131210, Add Start */
#define FTS_CTL_IIC
/* [OPT][Touch] Add for touch debug, 20131210, Add End */
...
#define TPD_INFO(fmt, arg...)  printk("[FOCAL][tpd info:5x06]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
//#define TP_DEBUG
#if defined(TP_DEBUG)
#define TPD_DEBUG(fmt, arg...)  printk("[FOCAL][tpd debug:5x06]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
#define TPD_DMESG(fmt, arg...)  printk("[FOCAL][tpd dmesg:5x06]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
#else
#define TPD_DEBUG(fmt, arg...)
#define TPD_DMESG(fmt, arg...)
#endif

/* [CCI] Touch - dynamic log - Start */
#define CCI_TOUCH_DYNAMIC_LOG

#ifdef CCI_TOUCH_DYNAMIC_LOG
enum {
	DYNAMIC_LOG_ERROR,
	DYNAMIC_LOG_INFO,
	DYNAMIC_LOG_DEBUG,
};
static int cci_dlog_mask = DYNAMIC_LOG_ERROR;

#define CCI_DLOGE(fmt, arg...) \
	if (cci_dlog_mask >= DYNAMIC_LOG_ERROR) printk("[FOCAL][error] %s: "fmt, __FUNCTION__, ##arg)
#define CCI_DLOGI(fmt, arg...) \
	if (cci_dlog_mask >= DYNAMIC_LOG_INFO) printk("[FOCAL][info] %s: "fmt, __FUNCTION__, ##arg)
#define CCI_DLOGD(fmt, arg...) \
	if (cci_dlog_mask >= DYNAMIC_LOG_DEBUG) printk("[FOCAL][debug] %s: "fmt, __FUNCTION__, ##arg)
#else
#define CCI_DLOGE(fmt, arg...)
#define CCI_DLOGI(fmt, arg...)
#define CCI_DLOGD(fmt, arg...)
...
#define CCI_LOG(fmt, arg...) printk("[FOCAL] %s: "fmt, __FUNCTION__, ##arg)
/* [CCI] Touch - dynamic log - End */
...
int fts_dma_i2c_read(struct i2c_client *client, u16 addr, int len, u32 rxbuf)
{
	...
	if (ret < 0)
		//dev_err(&client->dev, "[FOCAL] %s i2c dma read error.\n", __func__);
		CCI_DLOGE("i2c dma read error.\n");
...
static ssize_t fts_i2cdev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	//pr_debug("[FOCAL] i2c-dev: i2c-%d reading %zu bytes.\n",
	//	iminor(file->f_path.dentry->d_inode), count);
	CCI_DLOGD("i2c-dev: i2c-%d reading %zu bytes,\n",
		iminor(file->f_path.dentry->d_inode), count);
...
*/
	...
	//pr_debug("[FOCAL] i2c-dev: i2c-%d writing %zu bytes.\n",
	//	iminor(file->f_path.dentry->d_inode), count);
	CCI_DLOGD("i2c-dev: i2c-%d writing %zu bytes.\n",
		iminor(file->f_path.dentry->d_inode), count);
		...
		unsigned long arg)
{
	...
	if ((data_arg.size != I2C_SMBUS_BYTE) &&
		...
		//	"[FOCAL] size out of range (%x) in ioctl I2C_SMBUS.\n",
		//	data_arg.size);
		CCI_DLOGI("size out of range (%x) in ioctl I2C_SMBUS.\n",
			data_arg.size);
		return -EINVAL;
	}
	...
	if ((data_arg.read_write != I2C_SMBUS_READ) &&
		...
		//dev_dbg(&client->adapter->dev,
		//	"[FOCAL] read_write out of range (%x) in ioctl I2C_SMBUS.\n",
		//	data_arg.read_write);
		CCI_DLOGI("read_write out of range (%x) in ioctl I2C_SMBUS.\n",
			data_arg.read_write);
		return -EINVAL;
	}
	...
	if (data_arg.data == NULL) {
		//dev_dbg(&client->adapter->dev,
		//	"[FOCAL] data is NULL pointer in ioctl I2C_SMBUS.\n");
		CCI_DLOGI("data is NULL pointer in ioctl I2C_SMBUS.\n");
		return -EINVAL;
	}
...
static long fts_i2cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	CCI_DLOGD("ioctl, cmd=0x%02x, arg=0x%021x\n", cmd, arg);
...
static int fts_i2cdev_open(struct inode *inode, struct file *file)
{
	...
	if(ts_dev.ts){
		...
		//printk("[FOCAL][i2c-8] [fts_i2cdev_open][success]\n");
		CCI_DLOGI("success\n");
	}
	else{
		//printk("[FOCAL][i2c-8] [fts_i2cdev_open][fail]\n");
		CCI_DLOGI("fail\n");
		return -ENOMEM;
	}
...
int fts_i2cdev_init(struct fts_info *ts)
{
	...
	if(ret){
		...
		CCI_DLOGE("Unable to get a dynamic major for %s.\n", DRIVER_DEV_NAME);
		return ret;
	}
	...
	if (ret) {
		...
		CCI_DLOGE("Unable to register character device !\n");
		goto fail_add;
	}
	...
	if(IS_ERR(ts_dev.class)){
		...
		CCI_DLOGE("Unable to register i2c device class !\n");
	...
	if(IS_ERR(dev)){
		...
		CCI_DLOGE("Failed to create device !\n");
		ret = PTR_ERR(dev);
		goto err_device;
	}
...
static void sensitivity_set_func(struct work_struct *work)
{
	...
	if(ret < 0)
		...
		CCI_DLOGI("Can not write sensitivity\n");
	...
	if (EnableWakeUp) {
		...
		if(ret < 0)
			...
			CCI_DLOGI("Can not write scan rate\n");
#endif
	}
				...
				const char * buf, size_t n)
{
	...
	if (EnableWakeUp != symbol) {
		...
		if(ret < 0)
			...
			CCI_DLOGI("Can not write wake up mode\n");
	}
...
i2c_err:
	if (symbol == -1) {
		...
		CCI_DLOGI("touch sensitivity default value\n");
...
/* [CCI] Touch - dynamic log - Start */
#ifdef CCI_TOUCH_DYNAMIC_LOG
static ssize_t dynamic_log_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char * buf, size_t n)
{
	int symbol = -1;
	...
	symbol = myatoi(buf);
	if (symbol >= DYNAMIC_LOG_ERROR && symbol <= DYNAMIC_LOG_DEBUG) {
		CCI_LOG("set dynamic log (%d --> %d)\n", cci_dlog_mask, symbol);
		cci_dlog_mask = symbol;
	}
...
static ssize_t dynamic_log_show(struct kobject *kobj,
				struct kobj_attribute *attr,
				char * buf)
{
	return sprintf(buf, "%d\n", cci_dlog_mask);
...
/* [CCI] Touch - dynamic log - Start */
#ifdef CCI_TOUCH_DYNAMIC_LOG
static struct kobj_attribute dynamic_log_attr = { \
	.attr = { \
	.name = __stringify(dynamic_log), \
	.mode = 0644, \
	}, \
	.show = dynamic_log_show, \
	.store = dynamic_log_store, \
};
#endif
/* [CCI] Touch - dynamic log - End */
...
static  void tpd_down(int x, int y, int p) {
	...
	CCI_DLOGI("[Touch down] (x: %4d, y: %4d, p: %4d)\n", x, y, p);
 ...
 static  int tpd_up(int x, int y,int p) {
	...
	CCI_LOG("[Touch up] (x: %4d, y: %4d, p: %4d)\n", x, y, p);
	input_mt_sync(tpd->dev);
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	if (fts_dma_i2c_read(i2c_client, 0x00, 64, gpDMABuf_pa) < 0) {
		printk("[FOCAL][FTS]read touchinfo failed\n");
		return false;
	}
...
static int tpd_enable_ps(int enable)
{
	...
	TPD_PROXIMITY_DEBUG("[proxi_ft6306]read: 0xB0's value is 0x%02X\n", state);
	if (enable){
		state |= 0x01;
		tpd_proximity_flag = 1;
		TPD_PROXIMITY_DEBUG("[proxi_ft6306]ps function is on\n");
	}else{
		state &= 0x00;
		tpd_proximity_flag = 0;
		TPD_PROXIMITY_DEBUG("[proxi_ft6306]ps function is off\n");
	}

	ret = i2c_smbus_write_i2c_block_data(i2c_client, 0xB0, 1, &state);
	TPD_PROXIMITY_DEBUG("[proxi_ft6306]write: 0xB0's value is 0x%02X\n", state);
	return 0;
}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	TPD_PROXIMITY_DEBUG("[proxi_ft6306]command = 0x%02X\n", command);
	switch (command)
	{
		case SENSOR_DELAY:
...
static void tpd_calibrate_driver(int *x, int *y)
{
	...
	CCI_DLOGD("Call tpd_calibrate of this driver ..\n");
	...
	u8 state;
	 ...
	 do
	 {
		...
		if (tpd_proximity_flag == 1)
		{
			...
			{
				if (tpd_proximity_detect == 1) TPD_PROXIMITY_DEBUG("***** Relase *****");
				tpd_proximity_detect = 0;
			}
			else if (proximity_status == 0xE0)
			{
				if (tpd_proximity_detect == 0) TPD_PROXIMITY_DEBUG("***** Detect *****");
...
static void tpd_eint_interrupt_handler(void)
{
	...
	CCI_DLOGD("TPD interrupt has been triggered\n");
	tpd_flag = 1;
...
static int  tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
	...
	CCI_LOG("addr:0x%02x", i2c_client->addr);
	...
	if((i2c_smbus_read_i2c_block_data(i2c_client, FW_ID_ADDR, 1, &data))< 0)
	{
		...
		CCI_LOG("I2C transfer error, line: %d\n", __LINE__);
		return -1;
	}
#if 0
i2c_transfer_sucess:
	...
	if (!sensitivity_work) {
		...
		CCI_LOG("create work queue error, line: %d\n", __LINE__);
		return -1;
	}
	...
	if (!sensitivity_wq) {
		...
		CCI_LOG("create thread error, line: %d\n", __LINE__);
		return -1;
	}
	...
	if (IS_ERR(thread))
	{
		...
		CCI_LOG("failed to create kernel thread: %d\n", retval);
	}
	...
	{
		APS_ERR("proxi_5206 attach fail = %d\n", err);
	}
	else
	{
		APS_ERR("proxi_5206 attach ok = %d\n", err);
	}
#endif
...
/* [OPT][Touch] Add for touch debug, 20131210, Add Start */
	...
	if (ft_rw_iic_drv_init(i2c_client) < 0)
		CCI_LOG("%s:[FTS] create fts control iic driver failed\n");
#endif
/* [OPT][Touch] Add for touch debug, 20131210, Add End */
#ifdef FTS_APK_DEBUG
	...
	CCI_LOG("Touch Panel Device Probe %s\n", (retval < TPD_OK) ? "FAIL" : "PASS");
	CCI_LOG("[i2c-8] i2c_client->timing:%d\n", i2c_client->timing);
	return 0;
}
...
static int tpd_local_init(void)
{
	...
	CCI_LOG("Focaltech FT5x06 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
	...
	if(i2c_add_driver(&tpd_i2c_driver)!=0)
	{
		...
		CCI_LOG("unable to add i2c driver.\n");
		return -1;
	}
	...
	if(!gpDMABuf_va){
		...
		CCI_LOG("[error] Allocate DMA I2C Buffer failed!\n");
	}
...
static int tpd_resume(struct i2c_client *client)
{
	...
	if (tpd_proximity_flag == 1)
	{
		if (tpd_proximity_flag_one == 1)
		{
			...
			CCI_LOG("tpd_proximity_flag_one\n");
			return;
		}
	}
#endif
	...
	if (!EnableWakeUp) {
		...
		if(ret < 0)
			...
			CCI_LOG("Resume can not write 0xAB\n");
	}
	...
	if (!EnableWakeUp) {
		...
		*/
		...
		if(ret < 0)
			...
			CCI_LOG("Suspend can not write 0xAB\n");
	}
	return retval;
}
...
static int __init tpd_driver_init(void) {
	...
	CCI_LOG("MediaTek FT5x06 touch panel driver init\n");
	i2c_register_board_info(0, &i2c_tpd, 1);
	if(tpd_driver_add(&tpd_device_driver) < 0)
		//TPD_DMESG("add FT5x06 driver failed\n");
		CCI_LOG("add FT5x06 driver failed\n");
...
static void __exit tpd_driver_exit(void) {
	...
	CCI_LOG("MediaTek FT5x06 touch panel driver exit\n");
